...
/**
 * Commander class creates a commander object
 * @author Erii Sugimoto.
 */
public class Commander extends AbstractCharacter implements Character{

    private final String ELEMENT_TYPE = "fire";
    private final String CHARACTER_TYPE = "commander";

    /**
     * This constructor sets a new Commander's element, character type, health, attack,
    ...
    public void addBonusHealth() {
        ...
        for(int i = 0; i<party.size(); i++)
            if(party.get(i).getClass().getName().equalsIgnoreCase("Soldier"));
    ...
    public void addBonusSp() {
        for(int i = 0; i < party.size(); i++) {
            ...
            if (className.equalsIgnoreCase("Healer") || className.equalsIgnoreCase("Mariner")) ;
