...
/**
 * Huntress class creates an huntress object
 * @author Erii Sugimoto.
 */
public class Huntress extends AbstractCharacter implements Character{
    private final String ELEMENT_TYPE = "wind";
    private final String CHARACTER_TYPE = "huntress";

    /**
     * This constructor sets a new Assassin's element, character type, health, attack,
    ...
    public void addBonusHealth() {
        ...
        for (int i = 0; i < party.size(); i++)
            if (party.get(i).getClass().getName().equalsIgnoreCase("Assassin")) ;
    ...
    public void addBonusSp() {
        for (int i = 0; i < party.size(); i++) {
            ...
            if (className.equalsIgnoreCase("Engineer") || className.equalsIgnoreCase("Android")) ;
